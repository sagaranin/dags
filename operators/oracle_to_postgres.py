...
class OracleToPostgresOperator(BaseOperator):
        ...
        def execute(self, context):
            ...
            select_query = "select {fields} from {src_schema}.{src_table}".format(**self.kwargs)
            if self.kwargs['use_conditions']:
                select_query += " where {conditions}".format(**self.kwargs)
            ...
            tgt_cursor.execute("truncate table {tgt_schema}.{target_table_prefix}{tgt_table}".format(**self.kwargs))
            ...
            while True:
                ...
                if records:
                    execute_values(  # вставка батча данных
                        tgt_cursor,
                        "INSERT INTO {tgt_schema}.stg_{tgt_table} ({fields}) VALUES %s".format(**self.kwargs),
                        records
                    )
                else:
                    logging.info("Передача данных из таблицы {src_schema}.{src_table} завершена".format(**self.kwargs))
                    break
